...
class BookWormApp:
    def __init__(self, root):
        self.root = root
        self.root.title("BookWorm Reader")
        ...
        self.theme = 'bookworm'  # Custom theme name
    ...
    def check_for_badges(self, book_id):
        ...
        try:
            ...
            if books_read >= 5 and not self.has_badge("reader", "Bookworm"):
                self.award_badge("reader", "Bookworm", "Read 5 books")
    ...
    def show_about(self):
        ...
        if self.user_type == "reader":
            ...
            about_window.title("About BookWorm")
    ...
    def view_discussion_popup(self, book_title, discussion_id, parent_window=None):
        # Create popup window for discussion
        if parent_window:
            parent_window.destroy()
            
        discussion_window = tk.Toplevel(self.root)
        discussion_window.title(f"Discussion: {book_title}")
        discussion_window.geometry("800x600")
        discussion_window.configure(bg=self.colors["background"])
        
        # Create back button
        back_button = ttk.Button(discussion_window,
        ...
        # Create the comment function that will run when the button is clicked
        def add_comment_handler():
            ...
            if new_comment:
                try:
                    ...
                    self.show_toast("Comment added successfully", "success")
        ...
        # Create button with correct command
        comment_btn = tk.Button(
            add_comment_frame,
            text="Add Comment",
            command=add_comment_handler,
            bg=self.colors["button_primary"],
            fg="white",
            font=("Garamond", 12),
            relief=tk.FLAT
